...
export class CommanderStore {
  ...
  /**
   * Assigns a value directly to the store without adding the change to the undo/redo history.
   *
   * @param path - The path to the value in the store.
   * @param value - The value to assign.
   */
  assign(path: string[], value: DataValue): void {
  ...
  dispatch(event: CommandEvent): void {
    const { command, scope, isEffect, options } = event;
  ...
  clear(event?: CommandControlEvent): void {
    if (!event) {
      this.undoHistory = {};
      this.redoHistory = {};
      return;
