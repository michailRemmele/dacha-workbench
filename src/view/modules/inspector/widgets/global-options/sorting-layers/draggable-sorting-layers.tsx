...
export const DraggableSortingLayers: FC<DraggablePanelsProps> = ({
  ...
  const [draggablePanels, setDraggablePanels] = useState<SortingLayer[]>(sortingLayers)
  useEffect(() => setDraggablePanels(sortingLayers), [sortingLayers])

  const [activeSortingLayer, setActiveSortingLayer] = useState<SortingLayer | null>()

  const panelsIds = useMemo(() => draggablePanels.map((panel) => panel.id), [draggablePanels])

  const handleDragStart = useCallback((event: DragStartEvent) => {
    setActiveSortingLayer(draggablePanels.find((panel) => panel.id === event.active.id))
  }, [draggablePanels])
  ...
  return (
    ...
    >
      <SortableContext items={panelsIds} strategy={verticalListSortingStrategy}>
        {draggablePanels.map((panel) => (<DraggableSortingLayer key={panel.id} id={panel.id} />))}
      </SortableContext>
      <DragOverlay>
        {activeSortingLayer ? <DragOverlayView id={activeSortingLayer.id} /> : null}
      </DragOverlay>
    </DndContext>
  )
}
