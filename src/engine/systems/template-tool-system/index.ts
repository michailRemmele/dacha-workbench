...
export class TemplateToolSystem extends SceneSystem {
  ...
  constructor(options: SceneSystemOptions) {
    ...
    this.world.addEventListener(
      EventType.InspectedEntityChange,
      this.handleInspectEntity,
    );
    this.world.addEventListener(
      EventType.ToolCursorMove,
      this.handleToolCursorMove,
    );
    this.world.addEventListener(
      EventType.ToolCursorLeave,
      this.handleToolCursorLeave,
    );
    this.world.addEventListener(
  ...
  private handleAddFromTemplate = (event: MouseControlEvent): void => {
    ...
    this.cursor.x = x;
    this.cursor.y = y;
    updatePreviewPosition(this.cursor, this.world, this.preview);

    const tool = getTool(this.world);

    const templateId = tool.features.templateId.value as string | undefined;
    const nestToSelected = tool.features.nestToSelected.value as
      | string
      | undefined;
    ...
    const template = this.configStore.get([
      'templates',
      `id:${templateId}`,
    ]) as TemplateConfig;
    ...
    let actorPositionX = transform?.world.position.x ?? 0;
    let actorPositionY = transform?.world.position.y ?? 0;

    if (nestToSelected && this.selectedActorPath) {
      const selectedActorId = getActorIdByPath(
        this.selectedActorPath,
      ) as string;
      const parentActor = this.scene.findChildById(selectedActorId);
      const parentTransform = parentActor?.getComponent(Transform);

      const nestedPosition = parentTransform?.invertedWorldMatrix.apply({
        x: actorPositionX,
        y: actorPositionY,
      });

      actorPositionX = nestedPosition?.x ?? 0;
      actorPositionY = nestedPosition?.y ?? 0;
    }

    const path =
      nestToSelected && this.selectedActorPath
  ...
  private handleTemplatesUpdate = (path: string[]): void => {
    ...
    if (
      templateId &&
      templates.every((template) => template.id !== templateId)
    ) {
    ...
    if (
      !includesArray(path, [
        'templates',
        `id:${this.preview.templateId as string}`,
      ])
    ) {
      return;
    }
