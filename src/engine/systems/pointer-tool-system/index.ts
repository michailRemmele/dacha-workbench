import { SceneSystem, RendererAPI, ActorCollection, Transform } from 'dacha';
import type { World, SceneSystemOptions, Actor, ActorSpawner } from 'dacha';
...
export class PointerToolSystem extends SceneSystem {
  ...
  constructor(options: SceneSystemOptions) {
    ...
    const { world, scene, actorSpawner } = options;
    ...
    this.world.addEventListener(
      EventType.SelectEntitiesChange,
      this.handleSelectEntities,
    );
    this.world.addEventListener(
  ...
  private handleSelectionMoveStart = (event: MouseControlEvent): void => {
    ...
    const isWithinSelection =
      selectedActor &&
      actorPaths.some(
        (path) => getActorIdByPath(path, sceneId) === selectedActor.id,
      );

    if (isWithinSelection && shiftPick) {
      const newSelection = actorPaths.filter(
        (path) => getActorIdByPath(path, sceneId) !== selectedActor.id,
      );
    ...
    if (selectedActor && !isWithinSelection && shiftPick) {
      const newSelection = [
        ...actorPaths,
        buildActorPath(selectedActor, sceneId),
      ];
      this.sendSelectionEvents(newSelection, newSelection.at(-1));
      return;
    }
  ...
  private handleSelectionMoveEnd = (event: MouseControlEvent): void => {
    ...
    const newSelection = actors.reduce(
      (acc, actor) => {
  ...
  private sendSelectionEvents(
    selection: string[][],
    inspectedEntity: string[] | undefined,
  ): void {
  ...
  private updateFrames(): void {
    ...
    this.selectedActors.frames = this.selectedActors.actorPaths.reduce(
      (acc: Actor[], path) => {
